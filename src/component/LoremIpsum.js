...
const LoremIpsum = () =>{
    ...
    const btnClick = () =>{
        setCatchData(data.splice(0,inputData));
        setGetdata(true);
        if(inputData > 8){
    ...
    return(
        <>
            <div className="main">
                <h2 className="brown">TIRED OF BORING LOREM IPSUM?</h2>
                <div className="paraWithButton">
                    <h3>Paragraphs:</h3>
                    <input type="number" onChange={inputHandler}/>
                    <button className="orange" onClick={btnClick}>GENERATE</button>
                </div>
            </div>

            <div className="design">
                { getData && 
                    catchData.map((curElm) => (
                                <div>
                                    <p>{curElm}</p><br/>
                                </div>
                    ))
